...
def generate_scaffolds(dataset):
    ...
    for ind, smiles in enumerate(dataset.data.smiles):
...
def setup_scaffold_split(dataset):
    ...
    for scaffold_set in scaffold_sets:
        if len(train_index) + len(scaffold_set) > split_ratios[0] * len(dataset):
            if len(train_index) + len(val_index) + len(scaffold_set) > (split_ratios[0] + split_ratios[1]) * len(dataset):
                test_index += scaffold_set
            else:
                val_index += scaffold_set
        else:
            train_index += scaffold_set
    set_dataset_splits(dataset, [train_index, val_index, test_index])
