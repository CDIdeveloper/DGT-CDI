"""biodeg_gwu_no_ind PyG dataset — biodegradability (GWU batch 2, InD removed).

Sibling of ``biodeg_gwu.py``; same featurisation and split conventions, different
source parquets. This is the dataset the cross-model comparison in
``documents/dgt_porting_guide.md`` §1 is defined on:

    - inherently-biodegradable (InD) rows dropped (``cdi_class_gwu == 'InD'``),
      leaving binary labels 1 == RB (readily biodegradable), 0 == NRB.
    - id/metadata columns renamed ``*_gwu`` -> ``*_fromgwu`` upstream, so that
      **only real QM features end in ``_gwu``**. This matters for descriptor
      selection: ``_desc_select.select_descriptor_columns`` matches include /
      exclude patterns by *substring*, and ``'_gwu' in '_fromgwu'`` is False
      (the character before ``gwu`` is ``m``, not ``_``), so
      ``desc_include: ['_gwu']`` / ``desc_exclude: ['_gwu']`` resolve to exactly
      the QM / non-QM feature sets the porting guide defines by suffix.
    - the fixed train/test split is inherited from the parquets and must not be
      re-partitioned (guide §7 item 6).
...
consumed by ``preformat_BiodegGwuNoInd`` in master_loader.py to build the
...
class BiodegGwuNoInd(InMemoryDataset):
    """PyG InMemoryDataset for the biodeg_gwu_no_ind dataset.
    ...
    def download(self):
        ...
        if missing:
            raise FileNotFoundError(
                ...
                f"--dataset biodeg_gwu_no_ind --trans-learn-path <path>\n"
                f"(See documents/modeling_routine.md Step 0 for details.)"
            )
    ...
    def process(self):
        ...
        # Optional descriptor-column selection; effective desc_dim is the
        # SELECTED count, not the manifest's full count.
        ...
        # Optional descriptor standardisation: z-score with TRAIN-split stats
        # only (no val/test leakage), then persist stats + column names so
        # val/test/predict reuse identical normalisation.
        ...
        for split_source, df in (('train', df_train), ('test', df_test)):
            ...
            for row_idx in tqdm(
                range(len(df)),
                desc=f"biodeg_gwu_no_ind: featurising {split_source}",
            ):
                ...
                if feats is None:
                    logging.warning(
                        f"biodeg_gwu_no_ind: dropping unparseable SMILES "
                ...
                # never see it. The `line_graph_with_desc` head is the only
                # component that reads `batch.desc` — concatenating it with
                # the pooled atom/bond embeddings at readout.
        ...
        logging.info(
            f"biodeg_gwu_no_ind: featurised {len(data_list)} molecules "
    ...
    def __repr__(self):
        return f'BiodegGwuNoInd({len(self)})'
