...
from torch_geometric.datasets import (GNNBenchmarkDataset, Planetoid, TUDataset,
                                      WikipediaNetwork, ZINC, MoleculeNet, QM9)
...
def preformat_ZINC(dataset_dir, name):
    ...
    #     sub_datasets = []
    #     for split in ['train', 'val', 'test']:
    #         sub_dataset = ZINC(root=dataset_dir, subset=(name == 'subset'), split=split)
    #         sub_mols = list(SDMolSupplier(f"{dataset_dir}/raw/{name}_{split}_mmff.sdf"))
    #         sub_coordinates = np.concatenate([mol.GetConformer().GetPositions() for mol in sub_mols], axis=0)
    #         sub_coordinates = torch.tensor(sub_coordinates, dtype=torch.float)
    #         sub_dataset.data.pos = sub_coordinates
    #         sub_dataset.slices['pos'] = sub_dataset.slices['x']
    #         sub_datasets.append(sub_dataset)
    #     dataset = join_dataset_splits(sub_datasets)
    # if name == 'full':
    dataset = join_dataset_splits(
...
def preformat_MoleculeNet(dataset_dir, name):
    """Load and preformat MoleculeNet datasets.
    ...
    Returns:
        PyG dataset object
    """
    if name in ['BBBP', 'ClinTox', 'Tox21', 'ToxCast', 'SIDER',
                'HIV', 'BACE', 'MUV', 'PCBA',
                'ESOL', 'FreeSolv', 'Lipo']:
        return MoleculeNet(dataset_dir, name, transform=partial(typecast_x_and_edge_attr, type_str='float'))
...
def preformat_QM9(dataset_dir, name):
    ...
    dataset.data.pos = torch.tensor(pos, dtype=torch.float)
    dataset.slices['pos'] = dataset.slices['x']
    dataset.data.y = dataset.data.y[:, names.index(name)]
    if name in ['alpha']:
        dataset.data.y /= 10
    if name in ['u0', 'u', 'h', 'g']:
        dataset.data.y /= 100
