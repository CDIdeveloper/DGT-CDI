...
from graphgps.layer.dgt_layer import DGTLayer, NodeGTLayer, EdgeGTLayer
...
class PositionEncoder(torch.nn.Module):
    def __init__(self):
        ...
        self.dis_att_encoder = torch.nn.Linear(1, cfg.gnn.dim_inner)
        self.dis_val_encoder = torch.nn.Linear(1, cfg.gnn.dim_inner)
        self.cos_att_encoder = torch.nn.Linear(3, cfg.gnn.dim_inner)
        self.cos_val_encoder = torch.nn.Linear(3, cfg.gnn.dim_inner)
        self.bdl_encoder = torch.nn.Linear(1, cfg.gnn.dim_inner)
...
class NodeFeatureEncoder(torch.nn.Module):
    def __init__(self, dim_in):
...
@register_network('NodeGTModel')
class NodeGTModel(torch.nn.Module):
    def __init__(self, dim_in, dim_out):
        super().__init__()
        self.encoder = NodeFeatureEncoder(dim_in)
        dim_in = self.encoder.dim_in
...
class EdgeFeatureEncoder(torch.nn.Module):
    def __init__(self, dim_in):
...
@register_network('EdgeGTModel')
class EdgeGTModel(torch.nn.Module):
    def __init__(self, dim_in, dim_out):
        super().__init__()
        self.encoder = EdgeFeatureEncoder(dim_in)
        dim_in = self.encoder.dim_in
