...
def typecast_x_and_edge_attr(data, type_str):
    if type_str == 'float':
        data.x = data.x.float()
...
def compute_shortest_paths(data, config):
    ...
    S = A.clone() # shortest path matrix
    An = A.clone() # Random walk matrix
...
class LineData(Data):
    def __inc__(self, key, value, *args, **kwargs):
        if key == 'e_batch':
            return 1
        elif key == 'e2e_node_index':
            return self.x.size(0)
        elif key == 'e2e_edge_index':
            return self.edge_index[:, self.edge_index[0] < self.edge_index[1]].size(1)
        elif key == 'e2e_spd_index':
            return self.edge_index[:, self.edge_index[0] < self.edge_index[1]].size(1)
        elif key == 'e2e_ring_index':
            return self.edge_index[:, self.edge_index[0] < self.edge_index[1]].size(1)
        else:
            return super(LineData, self).__inc__(key, value, *args, **kwargs)

def line_graph(data):
    e2e_edge_index = []
    e2e_node_index = []
    undirected_edge_index = data.edge_index[:, data.edge_index[0] < data.edge_index[1]]
    undirected_edge_index = undirected_edge_index.transpose(1, 0).numpy().tolist()
    for i, edge1 in enumerate(undirected_edge_index):
        for j, edge2 in enumerate(undirected_edge_index):
            if (i < j) and (edge1[0] in edge2):
                e2e_edge_index.append((i, j))
                e2e_edge_index.append((j, i))
                e2e_node_index.append(edge1[0])
                e2e_node_index.append(edge1[0])
            if (i < j) and (edge1[1] in edge2):
                e2e_edge_index.append((i, j))
                e2e_edge_index.append((j, i))
                e2e_node_index.append(edge1[1])
                e2e_node_index.append(edge1[1])
    
    data.e_batch = torch.zeros((len(undirected_edge_index), ), dtype=torch.long)
    data.e2e_node_index = torch.LongTensor(e2e_node_index)
    data.e2e_edge_index = torch.LongTensor(e2e_edge_index).reshape(-1, 2).transpose(0, 1)
    
    # rwse
    kernel_param = cfg.posenc_RWSE.kernel
    if len(kernel_param.times) == 0:
        raise ValueError("List of kernel times required for RWSE")
    _, rw_landing_all = get_rw_landing_probs(ksteps=kernel_param.times,
                                             edge_index=data.e2e_edge_index,
                                             num_nodes=len(undirected_edge_index))
    data.e2e_edge_RWSE = rw_landing_all
    
    # spde
    data = compute_e2e_shortest_paths(data, cfg.dataset)

    # rings
    data = add_e2e_rings(data, cfg.dataset)
    
    data = LineData(**data.__dict__['_store'])
    return data

def compute_e2e_shortest_paths(data, config):
    from torch_geometric.utils import to_dense_adj, dense_to_sparse
    if data.e2e_edge_index.numel() == 0:
        N = data.e2e_node_index.shape[0]
        A = data.e2e_edge_index.new_zeros((N,N))
    else:
        A = to_dense_adj(data.e2e_edge_index).squeeze().long()
    S = A.clone() # shortest path matrix
    An = A.clone() # Random walk matrix
    # We choose to keep the torch geometric sparse format
    for step in range(1, config.spd_max_length):
        ...
        if (S > 0).all():
            break
    S = S.fill_diagonal_(0)
    data.e2e_spd_index, data.e2e_spd_lengths = dense_to_sparse(S)
    return data

def add_e2e_rings(data, config):
    # Find all pairs of nodes that are part of the same ring
    rings = get_rings(data.e2e_edge_index, max_k=config.rings_max_length)
    ring_connections = set()
    for ring in rings:
        for i in ring:
            for j in ring:
                if i < j:
                    ...
                    ring_connections.add((j, i))
    # Store these indices in a new ring_index graph attribute
    data.e2e_ring_index = data.e2e_edge_index.new_zeros(2, len(ring_connections))
    zipped_connections = list(zip(*ring_connections)) if len(ring_connections) > 0 else [(), ()]
    data.e2e_ring_index[0] = torch.Tensor(zipped_connections[0])
    data.e2e_ring_index[1] = torch.Tensor(zipped_connections[1])
    return data