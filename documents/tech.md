...
> **Observed (2026-06-07): molecule size sets the memory ceiling, not dataset size.** The `biodeg` baseline OOMed on a 14.6 GiB T4 at `batch_size=32` — the failure is in the **bond path** (`scores * E_val` over the `[B, M_max, M_max, D]` bond-bond tensor in [dgt_layer.py](../graphgps/layer/dgt_layer.py)), not the atom path. Because `M_max` is computed **per mini-batch**, a single large molecule inflates only the batch it lands in — but that one batch OOMing crashes the whole run, and with training-time shuffling the offending batch changes every epoch/seed, so the crash is **intermittent** (a tell-tale of a size outlier rather than a steady ceiling). Every other molecule in that batch is padded up to `M_max` too, so the largest member dominates the batch's cost. `biodeg` (avg ~18 atoms, 8054 mols) needs `batch_size=16` where `biodeg_gwu` (6042 mols) fit at `32` — the difference is the molecule-size distribution, not the row count. Mitigation order: lower `batch_size` (16 → 8; no cache invalidation needed) → if a few huge outliers exceed the SPDE-8 / rings-18 representational caps, consider a documented bond-count filter in the loader (mirrors the existing "drop unparseable SMILES with a logged warning" convention). Note: gradient accumulation does **not** help (the cost is a single batch's `M_max²` tensor, not accumulated gradients).
